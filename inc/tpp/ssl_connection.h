 ...
 */
TPP_EXPORT bool close_socket(tpp::socket sfd);
 ...
 * @return false on error, true on success
 */
TPP_EXPORT bool set_nonblocking(tpp::socket sockfd, bool non_blocking);
 ...
 */
class TPP_EXPORT ssl_connection {
 private:
  /**
 ...
 public:
   ...
   * @param plaintext_downgrade Set to true to connect using plaintext only,
   * without initialising SSL.
   * @param private_key if plaintext_downgrade is set to false, a private key
   * PEM file for SSL connections
   ...
   * read_loop() indicates setup failed
   */
  void read_loop();
  ...
  /**
   * @brief Handle input from the input buffer. This function will be called
   * until all data in the buffer has been processed and the buffer is empty.
   ...
   * @note The data may not be written immediately and may be written at a later
   * time to the socket.
   */
   ...
   */
  virtual void log(tpp::loglevel severity, const std::string &msg) const;
   ...
   */
  void on_read(tpp::socket fd, const struct tpp::socket_events &ev);
   ...
   */
  void on_write(tpp::socket fd, const struct tpp::socket_events &e);
