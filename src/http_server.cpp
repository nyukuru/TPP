...
http_server::http_server(conduit *owner, const std::string_view address, uint16_t port, http_server_request_event handle_request,
                         const std::string &private_key, const std::string &public_key)
    : socket_listener<http_server_request>(owner, address, port, private_key.empty() ? li_plaintext : li_ssl, private_key, public_key)
