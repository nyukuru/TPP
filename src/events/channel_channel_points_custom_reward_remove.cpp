...
void channel_channel_points_custom_reward_remove::handle(eventsub_client *client, nlohmann::json &j, const std::string &raw) const {
  conduit *creator = client->creator;
  if (creator->on_channel_channel_points_custom_reward_remove.empty()) {
    return;
  }

  uint32_t shard_id = client->shard_id;
  creator->enqueue_dispatch([creator, shard_id, j, raw]() mutable {
    channel_channel_points_custom_reward_remove_t event(creator, shard_id, raw);
    ...
    creator->on_channel_channel_points_custom_reward_remove.call(event);
  });
}
