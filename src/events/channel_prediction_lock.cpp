...
void channel_prediction_lock::handle(eventsub_client *client, nlohmann::json &j, const std::string &raw) const {
  conduit *creator = client->creator;
  if (creator->on_channel_prediction_lock.empty()) {
    return;
  }

  uint32_t shard_id = client->shard_id;
  creator->enqueue_dispatch([creator, shard_id, j, raw]() mutable {
    channel_prediction_lock_t event(creator, shard_id, raw);
    ...
    event.locked_at = string_not_null(&j, "locked_at");

    creator->on_channel_prediction_lock.call(event);
  });
}
