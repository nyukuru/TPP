...
int ssl_connection::start_connecting(tpp::socket            sockfd,
   ...
   * blocking connection - This is equivalent to EWOULDBLOCK on POSIX
   */
...
/* SSL Client constructor throws std::runtime_error if it can't allocate a
...
void ssl_connection::socket_write(const std::string_view data) {
  /* Because this is a non-blocking system we never write immediately. We append
   * to the buffer, which writes later.
   */
  ...
  if (status != 1) {
    ...
    switch (code) {
      case SSL_ERROR_NONE: {
        connected = true;
        socket_events se {*ev};
        se.flags = tpp::WANT_READ | tpp::WANT_WRITE | tpp::WANT_ERROR;
      ...
      case SSL_ERROR_WANT_READ: {
        socket_events se {*ev};
        se.flags = tpp::WANT_READ | tpp::WANT_ERROR;
        owner->socketengine->update_socket(se);
        break;
      }
      default: {
...
void ssl_connection::on_read(socket fd, const struct socket_events& ev) {
  ...
  if (plaintext && connected) {
    ...
    switch (e) {
      ...
      /* We get a WANT_WRITE if we're trying to rehandshake, and we block on a
       * write during that rehandshake. We need to wait on the socket to be
       * writeable but initiate the read when it is
       */
      case SSL_ERROR_WANT_WRITE: {
        socket_events se {ev};
        se.flags = WANT_READ | WANT_WRITE | WANT_ERROR;
...
void ssl_connection::on_write(socket fd, const struct socket_events& e) {
  ...
  if (!tcp_connect_done) {
    /* The first write event on an outbound TCP socket indicates connect() has
     * finished */
  ...
  if (!connected && plaintext) {
    ...
    if (ssl != nullptr && ssl->ctx != nullptr && ssl->ssl == nullptr) {
      ...
      if (ssl->ssl == nullptr) {
        throw tpp::connection_exception(err_ssl_new, "SSL_new failed!");
      }

      /* Associate the SSL session with the file descriptor, and set it as
       * connecting */
      ...
      if (this->is_server) {
         ...
         * This is needed for modern HTTPS and tells SSL which virtual host to
         * connect a socket to:
  ...
  if (connected) {
    ...
    if (plaintext) {
      ...
      /* SSL connections are more complex, the SSL_write function can return
       * some weirdness. See below. */
